...
for alpha = 1: 0.1: 1.7
    ...
    [dayAheadResult, caseOder,  T, vppNum] = dayaheadscheduling(1, alpha);
    ...
    % 记录
    recordI = recordI + 1;
    record(recordI, 1) = alpha;
    record(recordI, 2) = mean(sum(profitVppBefore));
    record(recordI, 3) = mean(sum(profitAddVpp));
end